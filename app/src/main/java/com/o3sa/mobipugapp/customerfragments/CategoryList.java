...
public class CategoryList extends Fragment {
    ...
    TextView sbcat_catgrylst_txt;
    RecyclerView subcat_recycleviwe;
    BasicComponents components;

    public ArrayList<HashMap<String, String>> ctgry_list;
    TextView nodata_txt;
    ...
    public void intialization(View v){
        ...
        components.CustomizeTextview(Sidemenu.menu_title, Constants.XXLarge4,R.color.txt_clr,getActivity().getResources().getString(R.string.category_lst),Constants.WrapLeftBold+Constants.Gibson, new int[]{0,0,0,0});
        StoredObjects.pagetype="categorylist";
        ...
        StoredObjects.Services_list.get(val).countDown = new CountDownTimer(60 * 1000, 1000) {
            public void onTick(long millisUntilFinished) {
                ...
                if(StoredObjects.Services_list.get(val).Result != ""){
                    ...
                    if(val==0) {
                        try {
                            ctgry_list = JsonParsing.GetJsonData(StoredObjects.Services_list.get(val).Result);

                            CustomRecyclerview recyclerview = new CustomRecyclerview(getActivity());
                            recyclerview.Assigndatatorecyleviewhashmap(subcat_recycleviwe, ctgry_list, "categry",
                                    Constants.Listview, 0, Constants.ver_orientation, R.layout.subcategory_listitems);
                            if(ctgry_list.size()==0){
